	...
	foreach ($batch as $tweet) {
        ...
        foreach ($preds as $ztable=>$keyword) {
			...
			for ($i = 0; $i < count($matches[0]);$i++) {
			    ...
			    echo $q_insert_mentions;
			    $r_insert = mysql_query($q_insert_mentions, $db->connection);
				}	
					// check against users in the table
					$q_check_user = "SELECT id from z_users_".$ztable." where id='".$tweet['from_user']."';";
					$res = mysql_query($q_check_user, $db->connection);
					if (mysql_num_rows($res) > 0)
					{
						$q_delete_user = "DELETE FROM z_users_".$ztable." where id='".$tweet['from_user']."';";
					...
					$curlData = current(json_decode($json, true));
					$fd = fopen("logs", "w");
					fputs($fd, $curlData);					
					//curl request
					/*$curl = curl_init();
